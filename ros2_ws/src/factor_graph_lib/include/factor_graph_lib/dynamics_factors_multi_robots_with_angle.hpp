#ifndef DYNAMICS_FACTORS_MULTI_ROBOTS_WITH_ANGLE_HPP
#define DYNAMICS_FACTORS_MULTI_ROBOTS_WITH_ANGLE_HPP
...
class LoadDynamicsMultiRobotsWithLoadOrientationFactor2: public NoiseModelFactor6<Vector6, Vector4, Vector1, Vector4, Vector1, Vector6> {
    double dt_;
    double load_mass_;
    double mu_;
    double mu2_;
...
public:
    LoadDynamicsMultiRobotsWithLoadOrientationFactor2(
        Key key_xl_k, 
        Key key_xr1_k, 
        Key key_tension1_k, 
        Key key_xr2_k, 
        ...
        NoiseModelFactor6<Vector6, Vector4, Vector1, Vector4, Vector1, Vector6>(
            model, 
            key_xl_k, 
    ...
    Vector evaluateErrorOnly(const Vector6& xl_k, 
        ...
        next_ang_vel(0) = xl_k(5) + dt_ / inertia_ * (t1 + t2 - mu2_ * load_mass_ * g_ * tanh(eps_ * xl_k(5))); 
    ...
    Vector evaluateError(const Vector6& xl_k, 
        ...
        std::vector<Vector4> xr_k = {xr1_k, xr2_k};
        std::vector<double> tension_k = {tension1_k(0), tension2_k(0)};
        //std::vector<gtsam::OptionalMatrixType> H = {H2, H3, H4, H5};
        ...
        if (H1) {
            ...
            Vector3 xl3_dxl(0.0, 0.0, 0.0), xl4_dxl(0.0, 0.0, 0.0), xl5_dxl(0.0, 0.0, 0.0);
            for (int i = 0; i < robot_num; i++) {
...
class LoadDynamicsMultiRobotsWithLoadOrientationFactor3: public NoiseModelFactorN<Vector6, Vector4, Vector1, Vector4, Vector1, Vector4, Vector1, Vector6> {
    double dt_;
    double load_mass_;
    double mu_;
    double mu2_;
...
public:
    LoadDynamicsMultiRobotsWithLoadOrientationFactor3(
        Key key_xl_k, 
        Key key_xr1_k, 
        Key key_tension1_k, 
        Key key_xr2_k, 
    ...
    Vector evaluateErrorOnly(const Vector6& xl_k, 
        ...
        Vector1 next_ang_vel(1);
        next_ang_vel(0) = xl_k(5) + dt_ / inertia_ * (t1 + t2 + t3 - mu2_ * load_mass_ * g_ * tanh(eps_ * xl_k(5))); 
    ...
    Vector evaluateError(const Vector6& xl_k, 
        ...
        std::vector<Vector4> xr_k = {xr1_k, xr2_k, xr3_k};
        std::vector<double> tension_k = {tension1_k(0), tension2_k(0), tension3_k(0)};
        //std::vector<gtsam::OptionalMatrixType> H = {H2, H3, H4, H5, H6, H7};
...
#endif // DYNAMICS_FACTORS_MULTI_ROBOTS_WITH_ANGLE_HPP