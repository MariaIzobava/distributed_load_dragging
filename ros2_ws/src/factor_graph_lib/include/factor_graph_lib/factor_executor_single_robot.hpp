...
public:
    ...
    FactorExecutorResult run(map<string, double>& factor_errors, double& pos_error) const override {
        ...
        const double robot_mass = 0.025; // kg  0.039 for real robot
        const double load_mass = 0.005;   // kg
        const double gravity = 9.81;
        const double mu = 0.3;
        const double half_size = 0.2; //0.2;
        const double real_cable_length = 1.03;  //1.03;


        const double cable_length = half_size + sqrt(real_cable_length * real_cable_length - (robot_height_ - half_size) * (robot_height_ - half_size));

        // VALUES TO TUNE
        // =============================
        // =============================
        const double u_upper_bound = getd("u_upper_bound", 0.4); //0.4
        ...
        double cable_length_offset = getd("cable_length_offset", 0.3); //0.021875); //0.0021875 good enough for 0.001 mass 0.38672
        ...
        if (have_u0_prior) {
          graph.add(PriorFactor<Vector2>(symbol_t('u', 0), Vector2::Zero(), control_init_cost));
        }
        if (have_t0_prior) {
        ...
        Values result = runOptimizer(debug_mode_, graph, initial_values, factor_errors, dt, mu, load_mass, gravity, 0.0, 0.0, 1);
